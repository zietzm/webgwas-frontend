...
export default function PhenotypeBuilder() {
    ...
    async function fetchFeatures(cohort: Cohort) {
        try {
            const myUrl = new URL(`${API_URL}/api/features`);
    ...
    function handleCohortSelect(cohort: Cohort): void {
        setSelectedCohort(cohort);
        setTree(rootNode);
        setValidationResult(null);
    };

    function handleAdd(parentNode: PhenotypeNode): void {
    ...
    async function validateTree(node: PhenotypeNode): Promise<ValidationResult> {
        const cohort_id = selectedCohort!.id.toString();
        const definition = convertToRPN(node);
        const myUrl = new URL(`${API_URL}/api/phenotype`);
        myUrl.searchParams.set('cohort_id', cohort_id);
        myUrl.searchParams.set('phenotype_definition', definition);
        ...
        if (!response.ok) {
            ...
            return { isValid: false, message: 'Failed to validate phenotype' };
        }
        setTree(node);
    ...
    async function handleValidate() {
        const result = await validateTree(tree);
        setValidationResult(result);
    };

    function convertToRPN(node: PhenotypeNode): string {
    ...
    async function handleRunGWAS() {
        ...
        try {
            ...
            const myUrl = new URL(`${API_URL}/api/igwas`);
            myUrl.searchParams.set('cohort_id', selectedCohort!.id.toString());
            myUrl.searchParams.set('phenotype_definition', convertToRPN(tree));
                ...
                <div className="flex flex-wrap gap-2">
                    {cohorts.map((cohort) => (
                        <button
                            ...
                            className={`py-2 px-4 rounded-full transition-colors ${selectedCohort === cohort
                ...
                <>
                    ...
                    <div className="flex flex-wrap gap-4 mb-6">
                        ...
                        <button
                            ...
                            className={`bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center transition-colors ${(!validationResult || !validationResult.isValid || jobStatus) ? 'opacity-50 cursor-not-allowed' : ''
                                }`}
